...
func _init(init_item: Item = null, init_size: int = 1):
	if not init_item:
		return
...
func combine(item_stack: ItemStack, set_if_empty: bool = false) -> bool:
	...
	var items_added = min(item_stack.size, item.max_stack_size - size)
